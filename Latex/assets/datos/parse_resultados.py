...
pol_pbs = grab(15,18,9,10,11)
for d in pol_pbs: d.update(tipo_muestra="retrospectiva", conservacion="pbs_1x")
pdf = pd.DataFrame(pol_pros+pol_sh+pol_pbs)
def parse_other(res):
    r=res.lower()
...
rsv_pbs = grab(33,36,9,10,11)
for d in rsv_pbs: d.update(tipo_muestra="retrospectiva", conservacion="pbs_1x")
rdf = pd.DataFrame(rsv_pros+rsv_sh+rsv_pbs)
