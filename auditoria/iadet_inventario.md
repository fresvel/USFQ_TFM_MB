...
| **I3** | Virus de interés: RSV | `01_introduccion.tex:67` | INTROC-RSV-* | anterior (`22275a8`) | P2 | «La caracterización genómica de RSV exige distinguir entre RSV-A y RSV-B…» | 🔁 aplicada: frase-marco → mecanismo directo (referencia no coincide → menos lecturas → genoma incompleto). Pendiente Turnitin |
| **I4** | Virus de interés: Enterovirus | `01_introduccion.tex:77` | INTROC-ENT-* | anterior (`86ee903`) | P4 | «El episodio más reciente lo describieron Klapsa… 118 aislamientos… En paralelo… Una revisión sistemática…» | 🔁 v2: NO borrar, TRAER contenido real (Klapsa: 21/52, 20/118, 6–10 VP1, recombinante especie C, Israel/EE.UU., IPV protege pero no frena excreción; Bottcher: proximidad entre países; Lesenfants: 26 eventos/21 países/6 regiones OMS). Pendiente Turnitin |
...
| ID | Subsección | Archivo:línea | ids trazab. | Origen | Patrón | Extracto (inicio) | Estado |
|---|---|---|---|---|---|---|---|
| **D1** | Linajes SARS y contexto epidemiológico | `05_discusion.tex:17` | DIS-LIN-* | anterior (`3584840`) | P4 | «El esfuerzo cayó a 744 genomas en 2024 y esa serie nacional termina ese año… no cubre 2025…» | ✅ CERRADO (v5, commit `639ec51`). Turnitin 35→28%, párrafo sale limpio. Estrategia validada: **registro de reporte** (autor+cifras, sin cierres interpretativos). v4 (alisado) había SUBIDO a 35% |
| **D2** | Enterovirus (poliovirus) | `05_discusion.tex:23` | DIS-ENT-* | **Lote 2** | P4 + glue interpretativo | «Ninguna de las 22 muestras rindió poliovirus… en Eslovaquia… En Canadá, en 2022… 99,4 %… 99,0 %… Seo…» | ✅ CERRADO (v3, commit `a341fd8`). DISCREC (recorte) confirma párrafo limpio. **Audit MSP_2023: Ecuador usa bOPV oral, NO vacuna inactivada** → claim falso corregido; excreción Sabin anclada a MasLago; voz del autor (n=9%/flujo/PTAR) |
| **D3** | RSV | `05_discusion.tex:27` (última oración) | DIS-RSV-* | anterior (`88d9712`) | P5 | «La asociación entre el ARN de RSV en aguas residuales y la circulación comunitaria proviene sobre todo de estudios de cuantificación…» | 🔁 v3 (v1/v2 no limpiaron). **Principio: Discusión = comparación ESPECÍFICA, no generalizada** (la generalización va en Conclusiones). Hughes head-to-head (τ 0,65–0,77 vs nuestras 4 muestras/EPI2ME). Pendiente Turnitin |
| **D4** | Conservación de muestras retrospectivas | `05_discusion.tex:29–33` (encabezado + 2 párrafos) | DIS-CONS-* | anterior (`210786e`) | P2 | «Conservación de muestras retrospectivas como factor crítico. Las ocho muestras retrospectivas, conservadas en DNA/RNA Shield 2X…» | 🔁 v3 REFORZADO: párr.1 Barbé (secuenciación fresco 94%→congelado 55%) + MejiaCalle + Parkins; párr.2 Zambrana (cuantificación RSV/PMMoV, storage significativo). Comparación específica, no adelgazada. Pendiente Turnitin |

## CONCLUSIONES — 3 párrafos detectados

| ID | Subsección | Archivo:línea | ids trazab. | Origen | Patrón | Extracto (inicio) | Estado |
|---|---|---|---|---|---|---|---|
| **C1** | Resumen (1.ª oración) | `06_conclusiones.tex:3` | CON-RES-* | línea tocada Lote 4; **oración de apertura anterior** | P5 | «En las aguas residuales de la PTAR Quitumbe, este estudio obtuvo evidencia molecular y genómica de los tres virus analizados, con un alcance que dependió de la carga viral recuperada en cada caso.» | 🔁 aplicada: apertura específica (mismo flujo a 3 virus, PTAR, 2025); quitado relleno «obtuvo evidencia… alcance según carga». Pendiente Turnitin |
| **C2** | Importancia y recomendaciones | `06_conclusiones.tex:11` (2.ª oración) | CON-IMP-* | anterior (`478b8cc`) | P5 | «La detección de los tres virus con un flujo común y la caracterización de un enterovirus de interés clínico confirman la viabilidad operativa…» | 🔁 aplicada: hecho concreto (Coxsackievirus A que AFP no registra) en vez de «confirman viabilidad/muestran valor». **Junto a C3** (bloque contiguo). Pendiente Turnitin |
| **C3** | Importancia y recomendaciones | `06_conclusiones.tex:13` (ambos párrafos de recomendación) | CON-IMP-* | anterior (`eaf065d`) | P4/P5 | «Para estudios futuros se recomienda iniciar con el procesamiento de las muestras de manera temprana… La incorporación de plataformas… El aumento del número de muestras…» | 🔁 aplicada: cada recomendación anclada a su hecho (8 retrosp. sin secuencia; SARS solo Cq bajos; RSV sin genoma). Sin «se recomienda/podría/fortalecerían… continua». **Junto a C2**. Pendiente Turnitin |
